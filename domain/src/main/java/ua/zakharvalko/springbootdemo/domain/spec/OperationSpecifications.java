    ...
    public Predicate toPredicate(Root<Operation> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        for (SearchCriteria criteria : list) {
            if (criteria.getOperation().equals(SearchOperation.EQUAL)) {
                ...
                predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get("date"), criteria.getValue().toString()));
            } else if (criteria.getOperation().equals(SearchOperation.BEFORE)) {
                predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get("date"), criteria.getValue().toString()));
            }
        }
